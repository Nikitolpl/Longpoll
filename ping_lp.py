...
async def ping_info(delay, peer_id, command):
    ...
    if command == "!н пинг" or command == "!н пиу" or command == "!н кинг" or command == "!н п" or command == "!лгбт пинг" or command == "!гей пинг":
        delta = vk_info.info_msg_date(peer_id)
        #c_time_str = str(datetime.fromtimestamp(round(c_time)))
        #v_time_str = str(datetime.fromtimestamp(round(event.msg['date'])))

        r_type = 'ПОНГ' if command == "!н пинг" else "ПАУ" if command == "!н пиу" else 'PONG' if command == "!н п" else "ЛГБТ" if command == "!лгбт пинг" else "КТО ПРОЧИТАЛ ТОТ ГЕЙ" if command == "!гей пинг" else "КОНГ"
