...
def templatess(templatess):
    templatess_set = {
            "name":name,
            "payload":data,
        }
    templatess = templatess + templatess_set
    return templatess
...
def attachments(peer_id):
    ...
    return attachments
'''
def parse_attachments(peer_id):
    history = vk.method('messages.getHistory', {'count': 1, 'peer_id': peer_id, 'rev': 0})
    attachments = history['items'][0]['attachments']['type']

    a_type = attachments
    " ".join(a_type)
    if a_type == "photo":
        owner_id = history['items']['items'][0]['attachments']['photo']['owner_id']
        ids = history['items']['items'][0]['attachments']['photo']['id']
        attachment = f"photo{owner_id}_{id}"
        return attachment
        self.attachments.append(
            f"photo{owner_id}_{id}"
        )
'''

async def create_templates(delay, peer_id, command) -> str:
